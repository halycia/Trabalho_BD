import { IsNotEmpty, IsNumber, IsString, Min, Max } from 'class-validator';
import { ApiProperty } from '@nestjs/swagger';

export class UpdateAvaliacaoDto {
  @ApiProperty({
    required: true,
    description: 'Nota da avaliação (1-5)',
    example: 4,
    minimum: 1,
    maximum: 5,
  })
  @IsNotEmpty()
  @IsNumber()
  @Min(1)
  @Max(5)
  nota: number;

  @ApiProperty({
    required: true,
    description: 'Texto atualizado',
    example: 'Mudei o texto',
  })
  @IsNotEmpty()
  @IsString()
  texto: string;

  @ApiProperty({
    required: true,
    description: 'Nova data da avaliação',
    example: '2024-01-16T10:30:00Z',
  })
  @IsNotEmpty()
  data_avaliacao: Date | string;

  @ApiProperty({
    required: true,
    description: 'Data do consumo do prato',
    example: '2024-01-05T12:00:00Z',
  })
  @IsNotEmpty()
  data_consumo: Date | string;

  @ApiProperty({
    required: true,
    description: 'ID do usuário que está atualizando a avaliação',
    example: 1,
  })
  @IsNotEmpty()
  @IsNumber()
  id_usuario: number;

  @ApiProperty({
    required: true,
    description: 'ID do prato da avaliação atualizada',
    example: 1,
  })
  @IsNotEmpty()
  @IsNumber()
  id_prato: number;
}
